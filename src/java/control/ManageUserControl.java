...
public class ManageUserControl extends HttpServlet {
    ...
    try {
        ...
        // Nếu không có action, hiển thị danh sách người dùng
        if (action == null) {
            List<Account> userList = dao.getAllUser();
            request.setAttribute("user", userList);
            request.getRequestDispatcher("ManagerUser.jsp").forward(request, response);
        }

        // Nếu là cập nhật quyền admin
            ...
            // Sau khi cập nhật, redirect để reload danh sách mới
